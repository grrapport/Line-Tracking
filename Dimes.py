...
def get_odds_by_league():
    dimes_response = str(requests.post(odds_url, headers=user_agent, data=data).content, 'utf-8')
    ...
    for tr in all_tr:
        ...
        if str(tr["class"]) == "[\'LHR\']":
            ...
            if temp_teams_date is None:
                continue
            if current_line is not None:
                lines.append(current_line)
                current_line = None
            if temp_teams_date[2] > datetime.datetime.now() + datetime.timedelta(days=3):
                break
